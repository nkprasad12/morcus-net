...
- [x] 🟢 **Define or replace the undefined `--pill-bg` design token.** (Completed)
  - Replaced `var(--pill-bg)` with `var(--tag-bg)` in [`core/components.css`](core/components.css) (`.btn-secondary:hover`) and [`dialog/dialog.css`](dialog/dialog.css) (`.dialog-close-btn:hover`).
- [x] 🟡 **Eliminate 4× theme block duplication in `shell/variables.css` (and fix missing `--border-mid`).** (Completed)
  - Combined `:root, :root[data-theme="light"], [data-theme="light"]` into a single selector block in both [`shell/variables.css`](shell/variables.css) and [`shell/critical_variables.css`](shell/critical_variables.css), guarded `@media (prefers-color-scheme: dark)` with `:root:not([data-theme="light"])`, added `--border-mid: #4d525c;` to `:root[data-theme="dark"], [data-theme="dark"]`, and added a token parity test suite in [`shell/variables.test.ts`](shell/variables.test.ts).
- [x] 🟡 **Scope `.lat-word`, `.section-anchor`, and `.badge` so Reader/Library rules do not clobber Dictionary styles.** (Completed)
  - Scoped `.lat-word` rules in [`reader/reader_text.css`](reader/reader_text.css) under `.reader-passage` so Dictionary word links retain `dict_typography.css` hover styling.
  - Scoped `.section-anchor` rules to `.reader-gutter .section-anchor` (with `:focus-visible` parity) in [`reader/reader_text.css`](reader/reader_text.css) and `.lsSenseBullet.section-anchor, .dict-layout .section-anchor, .reader-dict-panel .section-anchor` in [`dict/dict_typography.css`](dict/dict_typography.css).
  - Consolidated `.badge` and its modifiers (`.badge-pill`, `.badge-macra`, `.badge-trans`, `.badge-resume`) into [`core/components.css`](core/components.css) and removed duplicate rules from [`dict/dict_typography.css`](dict/dict_typography.css) and [`library/library.css`](library/library.css).
