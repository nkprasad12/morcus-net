...
describe("readFile", () => {
  ...
  test("parses chapters correctly", () => {
    ...
    expect(result.books[0].chapters[0].sections[0].passage).toBe(
      "Book1Chapter1Section1"
    );
    expect(result.books[0].chapters[0].sections[1].num).toBe("2");
    expect(result.books[0].chapters[0].sections[1].passage).toBe(
      "Book1Chapter1Section2"
    );
  });
  ...
  test("handles sections with gap elements", () => {
    ...
    expect(result.books[1].chapters[1].sections[1].passage).toBe(
      "Book2Chapter2Section2"
    );
  });
  ...
  test("handles chapters with only one section", () => {
    ...
    expect(result.books[1].chapters[0].sections[0].passage).toBe(
      "Book2Chapter1Section1"
    );
  });
});
