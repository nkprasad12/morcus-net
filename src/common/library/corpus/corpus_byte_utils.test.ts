...
describe("findFuzzyMatchesWithBitmaskAndArray", () => {
  it("should find fuzzy matches with direction right", () => {
    const bitmask = toBitMask([3, 8, 9], 32);
    const indices = [0, 11];
    const offset = 1;
    const maxDistance = 2;
    const direction = "right";
    // bitmask has [3, 8, 9]
    // indices are [0, 11]. with offset 1: [1, 12]
    // smeared indices (right, dist 2):
    // from 1: [1, 2, 3]
    // from 12: [12, 13, 14]
    // smeared indices: [1, 2, 3, 12, 13, 14]
    // intersection with bitmask: [3]
    const result = findFuzzyMatchesWithBitmaskAndArray(
      bitmask,
      indices,
      offset,
      maxDistance,
  ...
  it("should find fuzzy matches with direction left", () => {
    ...
    // bitmask has [3, 8, 9]
    // indices are [5, 11].
    // smeared indices (left, dist 2):
    // from 5: [3, 4, 5]
    // from 11: [9, 10, 11]
    // smeared indices: [3, 4, 5, 9, 10, 11]
    // intersection with bitmask: [3, 9]
    const result = findFuzzyMatchesWithBitmaskAndArray(
      bitmask,
      indices,
      offset,
  ...
  it("should find fuzzy matches with direction both", () => {
    const bitmask = toBitMask([3, 8, 9, 13], 32);
    const indices = [5, 11];
    const offset = 0;
    const maxDistance = 2;
    const direction = "both";
    // bitmask has [3, 8, 9, 13]
    // indices are [5, 11].
    // smeared indices (both, dist 2):
    // from 5: [3, 4, 5, 6, 7]
    // from 11: [9, 10, 11, 12, 13]
    // smeared indices: [3, 4, 5, 6, 7, 9, 10, 11, 12, 13]
    // intersection with bitmask: [3, 9, 13]
    const result = findFuzzyMatchesWithBitmaskAndArray(
      bitmask,
      indices,
      offset,
    ...
    expect(result).toEqual([3, 9, 13]);
  });

  it("should handle an offset", () => {
    const bitmask = toBitMask([13, 18, 19], 32);
    const indices = [5, 11];
    const offset = 10;
    const maxDistance = 2;
    const direction = "both";
    // bitmask has [13, 18, 19]
    // indices are [5, 11]. with offset 10: [15, 21]
    // smeared indices (both, dist 2):
    // from 15: [13, 14, 15, 16, 17]
    // from 21: [19, 20, 21, 22, 23]
    // smeared indices: [13, 14, 15, 16, 17, 19, 20, 21, 22, 23]
    // intersection with bitmask: [13, 19]
    const result = findFuzzyMatchesWithBitmaskAndArray(
      bitmask,
      indices,
      offset,
  ...
  it("should return an empty array when there are no matches", () => {
    const bitmask = toBitMask([100, 200], 256);
    const indices = [0, 10];
    const offset = 0;
    const maxDistance = 5;
    const direction = "both";
  ...
  it("should return an empty array for empty inputs", () => {
    const bitmask = toBitMask([], 32);
    const indices = [1, 2, 3];
    expect(
      findFuzzyMatchesWithBitmaskAndArray(bitmask, indices, 0, 2, "both")
    ).toEqual([]);

    const bitmask2 = toBitMask([1, 2, 3], 32);
    const indices2: number[] = [];
    expect(
      findFuzzyMatchesWithBitmaskAndArray(bitmask2, indices2, 0, 2, "both")
    ).toEqual([]);
  });
});
